...
begin
        ...
        if m is TProdBufEnumDescriptor then
         ...
         begin
          ...
          if not m.Forwarded then
           begin
            m.Forwarded:=true;
            Result:=Result+'  '+p.Prefix+m.PasName+
  ...
  Forwarded:=true;
  Result:=Result+'  '+p.Prefix+FPasName+' = class(TProtocolBufferMessage)'#13#10+
    '  private'#13#10;
  for i:=0 to FMembersIndex-1 do
    if FMembers[i].Quant<Quant_Repeated then
...
var
  ...
  s:string;
begin
  Result:='{ '+p.prefix+FPasName+' }'#13#10#13#10;
  if (FWireFlags and WireFlag_Default)<>0 then
   begin
  ...
  if (FWireFlags and WireFlag_Msg)<>0 then
   begin
    Result:=Result+'destructor '+p.Prefix+FPasName+'.Destroy;'#13#10;
  ...
  if (FWireFlags and WireFlag_VarInt)<>0 then
   begin
    Result:=Result+'procedure '+p.Prefix+FPasName+
  ...
  for i:=0 to FMembersIndex-1 do
   begin
    ...
    if FMembers[i].Quant>=Quant_Repeated then
      Result:=Result+
        'function '+p.Prefix+FPasName+'.Get'+FMembers[i].Name+
          '(Index: integer): '+FMembers[i].PascalType+';'#13#10+
        'begin'#13#10+
        '  Result := F'+FMembers[i].Name+'[Index];'#13#10+
        'end;'#13#10#13#10+
        'procedure '+p.Prefix+FPasName+'.Set'+FMembers[i].Name+
          '(Index: integer; '+s+'Value: '+FMembers[i].PascalType+');'#13#10+
        'begin'#13#10+
        ...
        'end;'#13#10#13#10+
        'procedure '+p.Prefix+FPasName+'.Add'+FMembers[i].Name+
